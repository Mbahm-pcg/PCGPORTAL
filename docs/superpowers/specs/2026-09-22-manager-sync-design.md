...
- **New account email:** the candidate's personal email from Paycor, if present. If Paycor has no personal email on file for that employee, this case is **not** pre-filled — it's surfaced as a plain heads-up for a human to sort out manually, never guessed (e.g. never falls back to the shared store inbox, since the outgoing manager may still read that).
...
- **Admin · Users page:** a small "N pending" indicator near the existing "Active" pill (`app.jsx:3757-3760`). Clicking it:
  - For a `replace` item: calls the existing `openEditPage(prefill)` (`app.jsx:3438`) with a synthetic object — `{ name, email: candidate.personalEmail, userType: 'manager', storePC, username: <suggested>, password: <generated> }` and **no `id` field**, which the existing form already treats as "create new" (confirmed: `setEditId(u ? u.id : null)` — an object with no `id` sets `editId` to `undefined`, same falsy path as the explicit `null` case). The admin reviews/edits and clicks the existing "Create User" button; the existing `sendWelcomeEmail` flow (`app.jsx:3369`) runs unchanged. On successful creation, the new user's `paycor_employee_id` is set to `candidate.employeeId`, and the pending item for that store is cleared.
...
- A `replace` candidate with no personal email on file is downgraded to a plain heads-up (like `needsReview`) rather than guessing an email to pre-fill.
