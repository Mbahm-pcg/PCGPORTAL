...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (action === 'seed-beverages') {
      ...
      if (!siteId || !token) return { statusCode: 500, headers, body: JSON.stringify({ error: 'Missing blob env vars' }) };
      ...
      return { statusCode: 200, headers, body: JSON.stringify({ ok: true, beverageCount: Object.keys(BEVERAGE_COSTS).length }) };
    }
    ...
    if (action === 'load-catalog') {
      ...
      return { statusCode: 200, headers, body: JSON.stringify({ ok: true, data: data || { beverages: BEVERAGE_COSTS, food: {}, iceCream: {} } }) };
    }
