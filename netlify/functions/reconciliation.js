// reconciliation.js — Sales reconciliation checker
...
exports.handler = async (event) => {
  ...
  if (action === 'snapshot') {
    const { busDt } = payload;
    if (!busDt) return { statusCode: 400, headers, body: JSON.stringify({ error: 'Missing busDt' }) };
    ...
    return { statusCode: 200, headers, body: JSON.stringify({ ok: true, busDt, storeCount: Object.keys(results).length }) };
  }
  ...
  if (action === 'compare') {
    ...
    if (!saved) return { statusCode: 404, headers, body: JSON.stringify({ error: `No snapshot found for ${busDt}. Take a snapshot first.` }) };
