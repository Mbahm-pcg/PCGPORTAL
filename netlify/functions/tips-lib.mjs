// tips-lib.mjs — Shared logic for the nightly Tips report, used by both the
// scheduled cron (tips-report-cron-background.mjs) and the on-demand manual
// trigger (tips-report-manual.mjs). Builds a per-transaction Excel breakdown
// grouped by district → store with store subtotals + a grand total, plus a
// "By Employee" sheet: each store's tip pool is divided by total hours worked
// that day (crew only) to get a per-hour rate, then multiplied by each
...
async function sendReportEmail(to, subject, html, buffer, filename) {
  ...
  if (nodemailer && process.env.GOOGLE_SMTP_USER) {
    try {
      ...
      console.warn('[tips-lib] SMTP failed:', e.message);
    }
  }
  ...
  if (process.env.RESEND_API_KEY) {
    try {
      ...
      console.warn('[tips-lib] Resend failed:', e.message);
    }
  }
...
export async function runTipsReport(busDt) {
  console.log(`[tips-lib] Building tips report for ${busDt} across ${STORES.length} stores`);
  ...
  for (let i = 0; i < STORES.length; i += BATCH) {
    ...
    await Promise.all(batch.map(async (s, j) => {
      ...
      try {
        ...
        console.error(`[tips-lib] ${s.name} checks error:`, err.message);
      }
      ...
      try {
        ...
        console.error(`[tips-lib] ${s.name} crew error:`, err.message);
      }
  ...
  console.log('[tips-lib] Email result:', emailResult, 'to', recipient);
  ...
  console.log('[tips-lib] done:', JSON.stringify(summary));
  return summary;
}
