...
// timezone marker (e.g. "2026-09-22T04:02:00"), unlike schedulingShifts' UTC "...Z" times.
// That naive string is Eastern WALL-CLOCK time, not UTC. Parsing it as UTC (the server runs
// in UTC) shifts every real on-time punch ~4-5 hours "earlier" (EDT/EST) — this produced a
// network-wide false "no clock-in" wave in shadow-mode testing. A string that already carries
// a zone ("Z" or +/-HH:mm) is trusted as-is; only a bare naive string gets this treatment.
...
function punchTimeMs(p) {
  const raw = p.punchDateTime || p.punchIn || p.inActualPunch || '';
  if (!raw) return NaN;
